 ...
 */
	public function admin_edit() {
		...
		if ($this->request->is(array('setting', 'put'))) {
			...
			// Convert the selected expiry time into a "-" delimited form
			$expiry = $this->request->data['Setting']['hashtagExpiry'];
			$convertedExpiry = $expiry["day"]."-".$expiry["month"]."-".$expiry["year"]."-".$expiry["hour"]."-".$expiry["min"];

			// Replace the array with the converted expiry to be saved as 'hashtagExpiry' in the Database
			$this->request->data['Setting']['hashtagExpiry'] = $convertedExpiry;
			...
			//var_dump($this->Setting->validationErrors);
