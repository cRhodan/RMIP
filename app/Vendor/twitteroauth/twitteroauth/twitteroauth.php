...
class TwitterOAuth {
  ...
  public $hostAPI = "https://api.twitter.com/1.1/";
   ...
   */
  function post($url, $parameters = array(), $isUpload = false, $multipart = false) {
    $response = $this->oAuthRequest($url, 'POST', $parameters, $multipart, $isUpload);
    if ($this->format === 'json' && $this->decode_json) {
      return json_decode($response);
    }
    return $response;
   ...
   */
  function oAuthRequest($url, $method, $parameters, $multipart = false, $isUpload = false) {
    ...
    if (strrpos($url, 'https://') !== 0 && strrpos($url, 'http://') !== 0) {
      if($isUpload){
        ...
        $url = "{$this->hostAPI}{$url}.{$this->format}";
      }
