...
NAV2D.Navigator = function(options) {
  ...
  this.rootObject = options.rootObject || new createjs.Container();
  var x_offset=-5.5; //negative
  var y_offset=26.15; //negative
  ...
  function sendGoal(pose) {
    ...
    goalMarker.x = pose.position.x-x_offset;
    goalMarker.y = -(pose.position.y-y_offset);
  ...
  poseListener.subscribe(function(pose) {
    ...
    robotMarker.y = -(pose.position.y-0.6);
    if (!initScaleSet) {
  ...
  if (withOrientation === false){
    ...
    var mouseEventHandler = function(event, mouseState) {
      ...
      if (mouseState === 'down'){
        ...
        //positionVec3.x=positionVec3.x+10;
